...
class OralDetailController extends GetxController {
  ...
  set currentPage4(int currentPage4) {}
  ...
  void getDetails() {
    try {
      variantList.clear();
      imgList2.clear();
      DioClient(myUrl: "products/${oralID}.json").getDetails().then((value) {
        ...
        if (value.statusCode == 200) {
          ...
          colorFlavour = (data['options'] as List)
              .firstWhere(
                  (option) => option['name'] == 'Color/Flavour')['values']
              .cast<String>(); // Cast to list of strings

          //Check if "Size" option exists before accessing its values
          var sizeOption = (data['options'] as List).firstWhere(
            (option) => option['name'] == 'Size',
            orElse: () => null,
          );
          if (sizeOption != null) {
            sizeName =
                sizeOption['values'].cast<String>(); // Cast to list of strings
          } else {
            // Handle the case where "Size" option is not present
            sizeName = [];
          }
          ...
          for (var d in varData) {
            ...
            map['option2'] = d['option2'];
            variantId = map['id'];
            var inventory_quantity = map['inventory_quantity'];
